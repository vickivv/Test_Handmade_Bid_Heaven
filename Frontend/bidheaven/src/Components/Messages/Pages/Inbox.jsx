...
function Inbox() {
  ...
  // Function to determine the sender's name
  const getSenderName = (message) => {
    if (message && message.sender_email && typeof message.sender_email === 'string') {
      return message.sender_email.includes('@handmade.com') ? 'Admin' : message.sender_email;
    }
    return '';
  };
  ...
  const displayedMessages = messages.filter((message) => user.email === message.receiver_email);
  ...
  const data = displayedMessages.slice((currentPage - 1) * messagesPerPage, currentPage * messagesPerPage).map(msg => ({
    key: msg.messageId,
    sender_email: getSenderName(msg),
        ...
        <div className="sent-container">
          ...
          {activeMessage && (
            <Modal
              ...
              footer={null}
            >
              <p>from: {activeMessage.receiver_email}</p>
